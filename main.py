...
def transplant():
    ...
    for orgin in orgin_data:
        ...
        if orgin[12] == 'D1：近段狭窄：':
            try:
                cur.execute(
                    'UPDATE record_gxb2 SET gmzy_d1jdxzd = %s WHERE ID = %s;', (orgin[14], table_gxb_id))
                conn.commit()
            except Exception as ex:
                logging.error('[更新异常]' + str(ex))
                conn.rollback()
        ...
        if orgin[12] == 'D1：远段狭窄：':
            try:
                cur.execute(
                    'UPDATE record_gxb2 SET gmzy_d1ydxzd = %s WHERE ID = %s;', (orgin[14], table_gxb_id))
                conn.commit()
            except Exception as ex:
                logging.error('[更新异常]' + str(ex))
                conn.rollback()
        ...
        if orgin[12] == 'D1：支架数量：':
            try:
                cur.execute(
                    'UPDATE record_gxb2 SET gmzy_d1zjs = %s WHERE ID = %s;', (orgin[14], table_gxb_id))
                conn.commit()
            except Exception as ex:
                logging.error('[更新异常]' + str(ex))
                conn.rollback()
                if error_stop:
                    break
        if orgin[12] == 'D1：_支架内再狭窄:':
            if orgin[14] == '支架内再狭窄':
                try:
                    cur.execute(
                        'UPDATE record_gxb2 SET gmzy_d1hzjzjnzxz = %s WHERE ID = %s;', (1, table_gxb_id))
                    conn.commit()
                except Exception as ex:
                    logging.error('[更新异常]' + str(ex))
                    conn.rollback()
                    if error_stop:
                        break
        if orgin[12] == 'D1：再狭窄程度：':
            try:
                cur.execute(
                    'UPDATE record_gxb2 SET gmzy_d1hzjzjnzxzcd = %s WHERE ID = %s;', (orgin[14], table_gxb_id))
                conn.commit()
            except Exception as ex:
                logging.error('[更新异常]' + str(ex))
                conn.rollback()
        ...
        if orgin[12] == 'E/A比值：':
            try:
                ...
                if error_stop:
                    break
        if orgin[12] == '超声诊断_超声诊断：':
            if orgin[14] == '左室舒张功能减退':
                try:
                    cur.execute(
                        'UPDATE record_gxb2 SET xzcszd_zsszgnjt = %s WHERE ID = %s;', (1, table_gxb_id))
                    conn.commit()
                except Exception as ex:
                    logging.error('[更新异常]' + str(ex))
                    conn.rollback()
            ...
            if orgin[14] == '陈旧性心肌梗死':
                try:
                    cur.execute(
                        'UPDATE record_gxb2 SET xzcszd_cjxxjgx = %s WHERE ID = %s;', (1, table_gxb_id))
                    conn.commit()
                except Exception as ex:
                    logging.error('[更新异常]' + str(ex))
                    conn.rollback()
            ...
            if orgin[14] == '室壁运动异常':
                try:
                    cur.execute(
                        'UPDATE record_gxb2 SET xzcszd_sbydyc = %s WHERE ID = %s;', (1, table_gxb_id))
                    conn.commit()
                except Exception as ex:
                    logging.error('[更新异常]' + str(ex))
                    conn.rollback()
            ...
            if orgin[14] == '主动脉瓣狭窄':
                try:
                    cur.execute(
                        'UPDATE record_gxb2 SET xzcszd_zdmbxz = %s WHERE ID = %s;', (1, table_gxb_id))
                    conn.commit()
                except Exception as ex:
                    logging.error('[更新异常]' + str(ex))
                    conn.rollback()
            ...
            if orgin[14] == '主动脉瓣关闭不全':
                try:
                    cur.execute(
                        'UPDATE record_gxb2 SET xzcszd_zdmbgbbq = %s WHERE ID = %s;', (1, table_gxb_id))
                    conn.commit()
                except Exception as ex:
                    logging.error('[更新异常]' + str(ex))
                    conn.rollback()
            ...
            if orgin[14] == '二尖瓣狭窄':
                try:
                    cur.execute(
                        'UPDATE record_gxb2 SET xzcszd_ejbxz = %s WHERE ID = %s;', (1, table_gxb_id))
                    conn.commit()
                except Exception as ex:
                    logging.error('[更新异常]' + str(ex))
                    conn.rollback()
            ...
            if orgin[14] == '二尖瓣脱垂':
                try:
                    cur.execute(
                        'UPDATE record_gxb2 SET xzcszd_ejbtc = %s WHERE ID = %s;', (1, table_gxb_id))
                    conn.commit()
                except Exception as ex:
                    logging.error('[更新异常]' + str(ex))
                    conn.rollback()
            ...
            if orgin[14] == '二尖瓣关闭不全':
                try:
                    cur.execute(
                        'UPDATE record_gxb2 SET xzcszd_ejbgbbq = %s WHERE ID = %s;', (1, table_gxb_id))
                    conn.commit()
                except Exception as ex:
                    logging.error('[更新异常]' + str(ex))
                    conn.rollback()
            ...
            if orgin[14] == '肺动脉瓣狭窄':
                try:
                    cur.execute(
                        'UPDATE record_gxb2 SET xzcszd_fdmbxz = %s WHERE ID = %s;', (1, table_gxb_id))
                    conn.commit()
                except Exception as ex:
                    logging.error('[更新异常]' + str(ex))
                    conn.rollback()
            ...
            if orgin[14] == '三尖瓣关闭不全':
                try:
                    cur.execute(
                        'UPDATE record_gxb2 SET xzcszd_sjbgbbq = %s WHERE ID = %s;', (1, table_gxb_id))
                    conn.commit()
                except Exception as ex:
                    logging.error('[更新异常]' + str(ex))
                    conn.rollback()
            ...
            if orgin[14] == '室壁瘤':
                try:
                    cur.execute(
                        'UPDATE record_gxb2 SET xzcszd_sbl = %s WHERE ID = %s;', (1, table_gxb_id))
                    conn.commit()
                except Exception as ex:
                    logging.error('[更新异常]' + str(ex))
                    conn.rollback()
            ...
            if orgin[14] == '附壁血栓':
                try:
                    cur.execute(
                        'UPDATE record_gxb2 SET xzcszd_fbxs = %s WHERE ID = %s;', (1, table_gxb_id))
                    conn.commit()
                except Exception as ex:
                    logging.error('[更新异常]' + str(ex))
                    conn.rollback()
            ...
            if orgin[14] == '升主动脉增宽':
                try:
                    cur.execute(
                        'UPDATE record_gxb2 SET xzcszd_szdmzk = %s WHERE ID = %s;', (1, table_gxb_id))
                    conn.commit()
                except Exception as ex:
                    logging.error('[更新异常]' + str(ex))
                    conn.rollback()
            ...
            if orgin[14] == '绝不担心':
                try:
                    ...
                    if error_stop:
                        break
        # GAD-7焦虑症筛查量表
        if orgin[12] == 'GAD-7焦虑症筛查量表?_1.感到不安、担心及烦躁：':
            if orgin[14] == '没有':
                try:
                    cur.execute(
                        'UPDATE record_gxb2 SET gdba = %s WHERE ID = %s;', (0, table_gxb_id))
                    conn.commit()
                except Exception as ex:
                    logging.error('[更新异常]' + str(ex))
                    conn.rollback()
            ...
            if orgin[14] == '有几天':
                try:
                    cur.execute(
                        'UPDATE record_gxb2 SET gdba = %s WHERE ID = %s;', (1, table_gxb_id))
                    conn.commit()
                except Exception as ex:
                    logging.error('[更新异常]' + str(ex))
                    conn.rollback()
            ...
            if orgin[14] == '一周以上时间':
                try:
                    cur.execute(
                        'UPDATE record_gxb2 SET gdba = %s WHERE ID = %s;', (2, table_gxb_id))
                    conn.commit()
                except Exception as ex:
                    logging.error('[更新异常]' + str(ex))
                    conn.rollback()
            ...
            if orgin[14] == '几乎每天':
                try:
                    cur.execute(
                        'UPDATE record_gxb2 SET gdba = %s WHERE ID = %s;', (3, table_gxb_id))
                    conn.commit()
                except Exception as ex:
                    logging.error('[更新异常]' + str(ex))
                    conn.rollback()
                    if error_stop:
                        break
        if orgin[12] == 'GAD-7焦虑症筛查量表?_2.不能停止担心或控制不了担心：':
            if orgin[14] == '没有':
                try:
                    cur.execute(
                        'UPDATE record_gxb2 SET bntzdx = %s WHERE ID = %s;', (0, table_gxb_id))
                    conn.commit()
                except Exception as ex:
                    logging.error('[更新异常]' + str(ex))
                    conn.rollback()
            ...
            if orgin[14] == '有几天':
                try:
                    cur.execute(
                        'UPDATE record_gxb2 SET bntzdx = %s WHERE ID = %s;', (1, table_gxb_id))
                    conn.commit()
                except Exception as ex:
                    logging.error('[更新异常]' + str(ex))
                    conn.rollback()
            ...
            if orgin[14] == '一周以上时间':
                try:
                    cur.execute(
                        'UPDATE record_gxb2 SET bntzdx = %s WHERE ID = %s;', (2, table_gxb_id))
                    conn.commit()
                except Exception as ex:
                    logging.error('[更新异常]' + str(ex))
                    conn.rollback()
            ...
            if orgin[14] == '几乎每天':
                try:
                    cur.execute(
                        'UPDATE record_gxb2 SET bntzdx = %s WHERE ID = %s;', (3, table_gxb_id))
                    conn.commit()
                except Exception as ex:
                    logging.error('[更新异常]' + str(ex))
                    conn.rollback()
                    if error_stop:
                        break
        if orgin[12] == 'GAD-7焦虑症筛查量表?_3.对各种各样的事情过度担心：':
            if orgin[14] == '没有':
                try:
                    cur.execute(
                        'UPDATE record_gxb2 SET gddx = %s WHERE ID = %s;', (0, table_gxb_id))
                    conn.commit()
                except Exception as ex:
                    logging.error('[更新异常]' + str(ex))
                    conn.rollback()
            ...
            if orgin[14] == '有几天':
                try:
                    cur.execute(
                        'UPDATE record_gxb2 SET gddx = %s WHERE ID = %s;', (1, table_gxb_id))
                    conn.commit()
                except Exception as ex:
                    logging.error('[更新异常]' + str(ex))
                    conn.rollback()
            ...
            if orgin[14] == '一周以上时间':
                try:
                    cur.execute(
                        'UPDATE record_gxb2 SET gddx = %s WHERE ID = %s;', (2, table_gxb_id))
                    conn.commit()
                except Exception as ex:
                    logging.error('[更新异常]' + str(ex))
                    conn.rollback()
            ...
            if orgin[14] == '几乎每天':
                try:
                    cur.execute(
                        'UPDATE record_gxb2 SET gddx = %s WHERE ID = %s;', (3, table_gxb_id))
                    conn.commit()
                except Exception as ex:
                    logging.error('[更新异常]' + str(ex))
                    conn.rollback()
                    if error_stop:
                        break
        if orgin[12] == 'GAD-7焦虑症筛查量表?_4.很紧张，很难放松下来：':
            if orgin[14] == '没有':
                try:
                    cur.execute(
                        'UPDATE record_gxb2 SET hnfsxl = %s WHERE ID = %s;', (0, table_gxb_id))
                    conn.commit()
                except Exception as ex:
                    logging.error('[更新异常]' + str(ex))
                    conn.rollback()
            ...
            if orgin[14] == '有几天':
                try:
                    cur.execute(
                        'UPDATE record_gxb2 SET hnfsxl = %s WHERE ID = %s;', (1, table_gxb_id))
                    conn.commit()
                except Exception as ex:
                    logging.error('[更新异常]' + str(ex))
                    conn.rollback()
            ...
            if orgin[14] == '一周以上时间':
                try:
                    cur.execute(
                        'UPDATE record_gxb2 SET hnfsxl = %s WHERE ID = %s;', (2, table_gxb_id))
                    conn.commit()
                except Exception as ex:
                    logging.error('[更新异常]' + str(ex))
                    conn.rollback()
            ...
            if orgin[14] == '几乎每天':
                try:
                    cur.execute(
                        'UPDATE record_gxb2 SET hnfsxl = %s WHERE ID = %s;', (3, table_gxb_id))
                    conn.commit()
                except Exception as ex:
                    logging.error('[更新异常]' + str(ex))
                    conn.rollback()
                    if error_stop:
                        break
        if orgin[12] == 'GAD-7焦虑症筛查量表?_5.非常焦躁，以至无法静坐：':
            if orgin[14] == '没有':
                try:
                    cur.execute(
                        'UPDATE record_gxb2 SET fcjz = %s WHERE ID = %s;', (0, table_gxb_id))
                    conn.commit()
                except Exception as ex:
                    logging.error('[更新异常]' + str(ex))
                    conn.rollback()
            ...
            if orgin[14] == '有几天':
                try:
                    cur.execute(
                        'UPDATE record_gxb2 SET fcjz = %s WHERE ID = %s;', (1, table_gxb_id))
                    conn.commit()
                except Exception as ex:
                    logging.error('[更新异常]' + str(ex))
                    conn.rollback()
            ...
            if orgin[14] == '一周以上时间':
                try:
                    cur.execute(
                        'UPDATE record_gxb2 SET fcjz = %s WHERE ID = %s;', (2, table_gxb_id))
                    conn.commit()
                except Exception as ex:
                    logging.error('[更新异常]' + str(ex))
                    conn.rollback()
            ...
            if orgin[14] == '几乎每天':
                try:
                    cur.execute(
                        'UPDATE record_gxb2 SET fcjz = %s WHERE ID = %s;', (3, table_gxb_id))
                    conn.commit()
                except Exception as ex:
                    logging.error('[更新异常]' + str(ex))
                    conn.rollback()
                    if error_stop:
                        break
        if orgin[12] == 'GAD-7焦虑症筛查量表?_6.变得容易烦恼或易被激怒：':
            if orgin[14] == '没有':
                try:
                    cur.execute(
                        'UPDATE record_gxb2 SET bdryfn = %s WHERE ID = %s;', (0, table_gxb_id))
                    conn.commit()
                except Exception as ex:
                    logging.error('[更新异常]' + str(ex))
                    conn.rollback()
            ...
            if orgin[14] == '有几天':
                try:
                    cur.execute(
                        'UPDATE record_gxb2 SET bdryfn = %s WHERE ID = %s;', (1, table_gxb_id))
                    conn.commit()
                except Exception as ex:
                    logging.error('[更新异常]' + str(ex))
                    conn.rollback()
            ...
            if orgin[14] == '一周以上时间':
                try:
                    cur.execute(
                        'UPDATE record_gxb2 SET bdryfn = %s WHERE ID = %s;', (2, table_gxb_id))
                    conn.commit()
                except Exception as ex:
                    logging.error('[更新异常]' + str(ex))
                    conn.rollback()
            ...
            if orgin[14] == '几乎每天':
                try:
                    cur.execute(
                        'UPDATE record_gxb2 SET bdryfn = %s WHERE ID = %s;', (3, table_gxb_id))
                    conn.commit()
                except Exception as ex:
                    logging.error('[更新异常]' + str(ex))
                    conn.rollback()
                    if error_stop:
                        break
        if orgin[12] == 'GAD-7焦虑症筛查量表?_7.感到好像有什么可怕的事会发生：':
            if orgin[14] == '没有':
                try:
                    cur.execute(
                        'UPDATE record_gxb2 SET gdysqfs = %s WHERE ID = %s;', (0, table_gxb_id))
                    conn.commit()
                except Exception as ex:
                    logging.error('[更新异常]' + str(ex))
                    conn.rollback()
            ...
            if orgin[14] == '有几天':
                try:
                    cur.execute(
                        'UPDATE record_gxb2 SET gdysqfs = %s WHERE ID = %s;', (1, table_gxb_id))
                    conn.commit()
                except Exception as ex:
                    logging.error('[更新异常]' + str(ex))
                    conn.rollback()
            ...
            if orgin[14] == '一周以上时间':
                try:
                    cur.execute(
                        'UPDATE record_gxb2 SET gdysqfs = %s WHERE ID = %s;', (2, table_gxb_id))
                    conn.commit()
                except Exception as ex:
                    logging.error('[更新异常]' + str(ex))
                    conn.rollback()
            ...
            if orgin[14] == '几乎每天':
                try:
                    cur.execute(
                        'UPDATE record_gxb2 SET gdysqfs = %s WHERE ID = %s;', (3, table_gxb_id))
                    conn.commit()
                except Exception as ex:
                    logging.error('[更新异常]' + str(ex))
                    conn.rollback()
...
def update_gxb():
    update_args = ['wh_wh', 'wh_eh', 'wh_ef', 'wh_hz', 'wh_fl', 'wh_szbw', 'wh_sizbw', 'wh_sdh', 'wh_tl', 'wh_bl',
                   ...
                   'gmcta_lcxzd', 'gmcta_lcxyd', 'gmcta_rcajd', 'gmcta_rcazd', 'gmcta_rcayd', 'gmcta_d1hzzzjd',
                   'gmcta_d1hzzzyd', 'gmcta_qtfzzzxzwz', 'gmzy_lmzjs', 'gmzy_lmzjnzxz', 'gmzy_ladzjs', 'gmzy_ladzjnzxz',
                   'gmzy_lcxzjs', 'gmzy_lcxzjnzxz', 'gmzy_rcazjs', 'gmzy_rcazjnzxz', 'gmzy_d1zjs', 'gmzy_d1hzjzjnzxz',
